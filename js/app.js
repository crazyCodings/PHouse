...
(function($, owner) {
	...
	owner.login = function(loginInfo, callback) {
		...
		if (loginInfo.account.length < 5) {
			return callback('账号最短为 6个字符');
		}
		if (loginInfo.password.length < 6) {
			return callback('密码最短为 6 个字符');
		}
		if(loginInfo.password != "123456"){
			return callback('帐号或密码不正确');
		}
		...
		mui.ajax(url,{
			...
			catche : false,
			ifModified : true,
			timeout:3000,		//超时时间设置为2秒；
			headers:{'appId':serviceinfo.appid,
